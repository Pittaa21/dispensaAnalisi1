...
\section{Limiti di funzioni}
    ...
    \newpage
    \subsection{Sviluppi asintotici di alcune funzioni}\vspace{10pt}\large
    \begin{center}
        {\rowcolors{1}{gray!10}{white!100}
        \begin{tabular}{| c | c |}
            \hline
    ...
    \begin{align*}
        ...
        =&\underbrace{\frac{n!}{n!}\cdot x_0^n\cdot1}_{x_0^n}+\underbrace{\frac{n!}{(n-1)!}\cdot x_0^{n-1}\cdot h}_{n\cdot x_0^{n-1}\cdot h^2}+\underbrace{\frac{n!}{2(n-2)!}\cdot x_0^{n-2}\cdot h^2}_{\frac{1}{2}n(n-1)\cdot x_0^{n-2}\cdot h^2}\qquad\text{nel caso di }n=2\text{ (per gli altri valori è analogo)}\\
        =&\lim_{h\rightarrow0}\frac{x^n_0+n\cdot x_0^{n-1}\cdot h+\frac{1}{2}n(n-1)\cdot x_0^{n-2}\cdot h^2-x^n_0}{h}=\\
    ...
    \end{align*}
        \newpage
        \subsection{Tabella Derivate Semplici}\vspace{10pt}\begin{center}\large
        {\rowcolors{1}{gray!10}{white!100}
        \begin{tabular}{| c | c |}
            \hline
    ...
    \colorbox{yellow}{Osservazione}: $f$ derivabile in $x_0\Rightarrow f$ e $x_0\nLeftarrow f$ continua in $x_0$\\
    ...
    \begin{defi}{Punti di non derivabilità}{}
        ...
        \[f^1_+(x_0)\ne f^1_-(x_0)\]
        E almeno una delle due sia finita, allora $x_0$ si dice \underline{punto angoloso}
    \end{defi}
    \vspace{20pt}
    \begin{defi}{}{}
        $f:\ I\rightarrow\mathbb{R},\ I$ intervallo aperto, $x_0\in I$ e si indica:
        \[f^1(x_0)=\pm\infty\]
        Allora $x_0$ si dice punto di \underline{flesso a tangente verticale}\\
    ...
    \begin{theo}{Teorema di Fermat o del punto critico}{}
        $f:\ ]a,b[\rightarrow\mathbb{R}\qquad x_0\in\ ]a,b[$, $f$ derivabile in $x_0$, $f$ ha estremo locale in $x_0$. Allora $f^1(x_0)=0$
    \end{theo}
    ...
    Osservo\[\frac{f(x)-f(x_0)}{x-x_0}\qquad\le0\ x>x_0\qquad\ge0\ x<x_0\]
    ...
    \begin{theo}{Teorema di Rolle}{}
        ...
        \begin{enumerate}
            ...
            \item $f(a)=f(b)$
        \end{enumerate}
        Allora esiste $c\in\ ]a,b[$ tale che $f^1(c)=0$
    \end{theo}
    \colorbox{yellow}{Dim}: Se $f$ non c'è nulla da dimostrare poichè $f^1(c)=0$.\\
    Suppongo che $f$ non sia costante, per il teorema di Weistrass ha max e min in $[a,b]$ tali che
    \[f(x_m)\le f(x)\le f(x_M)\qquad\forall\ x\in\ [a,b]\]
    È necessario dimostrare che $x_m$ o $x_M$ siano all'interno dell'intervallo aperto a,b. Perchè per il teorema di Fermat
    $f^1$ si annulla in tali punti.\\\\
    Suppongo per assurdo che $x_m$ e $x_M$ appartengono agli estremi a e b compresi. Di conseguenza ottengo che, per il terzo punto del teorema di Rolle, 
    $f(a)=f(b)$, quindi, $f(x_m)=f(x_M)$ ciò implica che $f$ è continua, ma all'inizio abbiamo considerato $f$ come non continua.\\\\
    ...
    \begin{theo}{Teorema di Cauchy}{}
        ...
        \[g^1(c)[f(b)-f(a)]=f^1(c)[g(b)-g(a)]\]
        Se $g^1(x)\ne0$ vale:
        \[\frac{f(b)-f(a)}{g(b)-g(a)}=\frac{f^1(c)}{g^1(c)}\]
    \end{theo}
    ...
    \begin{theo}{Monotonie}{}
        ...
        \begin{enumerate}
            ...
            \item Se $f^1(x)>0\quad\forall\ x\in\ ]a,b[\ \Rightarrow f$ è strett crescente
            \item Se $f^1(x)<0\quad\forall\ x\in\ ]a,b[\ \Rightarrow f$ è strett decrescente
        \end{enumerate}
    \end{theo}
    ...
    ho $\frac{f(y)-f(x)\ge0}{y-x>0}=f^1(c)\quad \ge0$ quindi se faccio il limite per $y\rightarrow x^+$ ottengo che $f^1_+(x)\ge0$ se ora applico il ragionamento per 
    ogni $x$ ottengo che $f^1(x)\ge0$. \checkmark\\\\
    ...
    \newpage
        \subsection{Sviluppi di McLaurin}\vspace{10pt}\large\begin{center}
        {\rowcolors{1}{gray!10}{white!100}
        \begin{tabular}{| c | c | c |}
            \hline
    ...
    \end{defi}\vspace{20pt}
        \subsection{Integrali indefiniti noti}\vspace{10pt}\large\begin{center}
        {\rowcolors{1}{gray!10}{white!100}
        \begin{tabular}{| c | c |}
            \hline
